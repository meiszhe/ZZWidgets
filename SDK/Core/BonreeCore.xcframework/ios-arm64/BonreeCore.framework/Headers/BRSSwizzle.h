 ...
 */
typedef void (*BRSSwizzleOriginalIMP)(void /* id, SEL, ... */ );

/**
 在新实现的block中，BRSSwizzleInfo被用于获取被交换的方法的原始实现，以便在新实现中调用原始实现。
 */
@interface BRSSwizzleInfo : NSObject

/// 返回被交换的方法的原始实现
-(BRSSwizzleOriginalIMP)getOriginalImplementation;
 ...
 */
typedef id (^BRSSwizzleImpFactoryBlock)(BRSSwizzleInfo *swizzleInfo);

@interface BRSSwizzle : NSObject
...
+ (BOOL)swizzleInstanceMethod:(SEL)selector
         ...
         typeEncodingProvider:(Method)typeEncodingProvider
          destinationClsssPtr:(Class *)destinationClassPtr
                          key:(const void *)key
                newImpFactory:(BRSSwizzleImpFactoryBlock)factoryBlock;
